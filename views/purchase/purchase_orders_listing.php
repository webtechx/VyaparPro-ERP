...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-xl-12">
            ...
            $next_po_number = 'PO-' . ($_SESSION['organization_short_code'] ?? 'ORG') . '-0001';
            $org_id = $_SESSION['organization_id'] ?? 0;
            $org_short_code = $_SESSION['organization_short_code'] ?? 'ORG';
            
            $po_sql = "SELECT po_number FROM purchase_orders WHERE organization_id = '$org_id' ORDER BY purchase_orders_id DESC LIMIT 1";
            $po_res = $conn->query($po_sql);
            if ($po_res && $po_res->num_rows > 0) {
                $last_po = $po_res->fetch_assoc()['po_number'];
                // Extract number, increment, format
                // Check if last PO matches the new format
                if (preg_match('/PO-' . preg_quote($org_short_code, '/') . '-(\d+)/', $last_po, $matches)) {
                    $next_num = intval($matches[1]) + 1;
                    $next_po_number = 'PO-' . $org_short_code . '-' . str_pad($next_num, 4, '0', STR_PAD_LEFT);
                } 
                // Fallback for transition from old format PO-XXXX to PO-ORG-XXXX (optional: or start from 0001)
                // If the last PO was just PO-0001 (old format), we start new format at 0001 or continue? 
                // Usually changing format implies starting a new sequence or resetting to 1 for that unique prefix.
                // We'll stick to the generated default (PO-ORG-0001) if regex doesn't match.
            }
            ?>
                ...
                <div class="card-body">
                    <form id="po_form" action="<?= $basePath ?>/controller/purchase/add_purchase_order.php" method="post" enctype="multipart/form-data">
                            ...
                            <div class="col-md-6">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row mb-2 align-items-center">
                                            <div class="col-8">
                                                <div class="input-group input-group-sm">
                                                    <span class="input-group-text" style="width: 100px;">GST</span>
                                        ...
                                        <div class="row mb-2 align-items-center">
                                            <div class="col-8">
                                                <div class="input-group input-group-sm">
                                                    <span class="input-group-text" style="width: 100px;">Adjustment</span>
                        ...
                        <div class="mt-4 text-end">
                            ...
                            <button type="submit" name="save_po" id="submit_btn" value="draft" class="btn btn-info">Save as Draft</button>
                            <button type="submit" name="save_po" id="save_sent_btn" value="sent" class="btn btn-primary">Save & Send</button>
            ...
            <div class="card" id="po_list_card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        <button type="button" id="add_po_btn" class="btn btn-primary btn-sm" <?= can_access('purchase_orders', 'add') ? '' : 'disabled title="Access Denied"' ?>>New Purchase Order</button>
                    </div>
                </div>
                <div class="card-body">
                    <table data-tables="basic" class="table table-hover table-striped dt-responsive align-middle mb-0" id="purchase_orders_table" style="width: 100%;">
                        <thead class="table-light">
                            <tr>
                                <th>Date</th>
                                <th>PO Number</th>
                        ...
                        <tbody>
                            <?php
                            $sql = "SELECT po.*, v.display_name, v.avatar, o.organizations_code
                                    ...
                                    WHERE 1=1 ORDER BY po.purchase_orders_id DESC";
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                if (!empty($vendorAvatar)) {
                                                     // If stored path is full path (new vendors)
                                                     if(strpos($vendorAvatar, 'uploads/') !== false) {
                                                         $avatarSrc = $basePath . '/' . $vendorAvatar;
                                                     } else {
                                                         // Fallback to constructing it if just filename (legacy)
                                                ...
                                                ?>
                                                <div>
                                                    <?= htmlspecialchars(ucwords(strtolower($vendorName))) ?>
                                                </div>
                                            </div>
                                        </td>
                                        <td><span class="badge bg-<?= $statusClr ?> text-uppercase"><?= $row['status'] ?></span></td>
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item  <?= can_access('purchase_orders', 'edit') ? '' : 'disabled' ?>" 
                                                            href="<?= can_access('purchase_orders', 'edit') ? $basePath . '/controller/purchase/send_purchase_order.php?id=' . $row['purchase_orders_id'] : 'javascript:void(0);' ?>" 
                                                            <?= can_access('purchase_orders', 'edit') ? "onclick=\"return confirm('Confirm Send? this will update status to Sent')\"" : 'title="Access Denied"' ?>>
                                                            <i class="ti ti-send me-2"></i>Send
                                                        </a>
                                                    </li>
                                                    <?php endif; ?>
                                                    
                                                    <?php if(!in_array($row['status'], ['sent', 'confirmed', 'partially_received', 'received'])): ?>
                                                    <li>
                                                        <a class="dropdown-item <?= can_access('purchase_orders', 'edit') ? '' : 'disabled' ?>" 
                                                           href="javascript:void(0)" 
                                                           <?= can_access('purchase_orders', 'edit') ? 'onclick="editPO(' . $row['purchase_orders_id'] . ')"' : 'title="Access Denied"' ?>>
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item  <?= can_access('purchase_orders', 'delete') ? 'text-danger' : 'disabled' ?>" 
                                                           href="<?= can_access('purchase_orders', 'delete') ? $basePath . '/controller/purchase/delete_purchase_order.php?id=' . $row['purchase_orders_id'] : 'javascript:void(0);' ?>" 
                                                           <?= can_access('purchase_orders', 'delete') ? "onclick=\"return confirm('Confirm delete?')\"" : 'title="Access Denied"' ?>>
...
/* ================================
   BUTTON EVENTS
================================ */
document.getElementById('add_po_btn').addEventListener('click', () => {
    resetForm();
    toggleView(true);

    // IMPORTANT: init vendor AFTER form is visible
    setTimeout(() => {
        initVendorSelect2();
    }, 100);
});
...
function addItemRow(data = null){
    ...
    row.innerHTML = `
        ...
        <td>
            <input type="number" name="items[${index}][rate]" class="form-control form-control-sm rate-input text-end" value="${data ? data.rate : 0}" step="0.01" oninput="calculateRow(this)">
        </td>
     <td>
            <div class="input-group input-group-sm">
...
function editPO(id) {
    ...
    document.getElementById('save_sent_btn').classList.add('d-none');
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // Note: po object now contains avatar, vendor_email, vendor_company from get_purchase_order_details.php update
            ...
            // Populate Attachments
             $('#existing_attachments').remove(); // Clear previous
             if(files && files.length > 0){
                 const orgCode = '<?= $_SESSION['organization_code'] ?? '' ?>'; 
                 let fileLinks = '';
                 files.forEach(file => {
                     // Path: uploads/ORG_CODE/purchase_orders/FILE_NAME
                     // We need org code. If $_SESSION is available in PHP render, we use it. 
                     // Otherwise we might need it from response if user viewing another org's data (super admin context?), 
                     // but likely current session org is fine for now or response.org_code
                     
                     // Use specific org code from response if available (multi-tenant safety), else session
                     // Note: get_purchase_order_details doesn't seemingly return org_code in 'po' widely? 
                     // Let's assume session or add it to controller if needed. 
                     // Actually controller returns po.* so if po has organization_id we might need lookup or just rely on session.
                     
                     const filePath = basePath + '/uploads/' + orgCode + '/purchase_orders/' + file.file_name;
                     fileLinks += `<div class="mb-1">
            ...
            // Special logic for Cancelled POs
            if (po.status === 'cancelled') {
                document.getElementById('save_sent_btn').classList.remove('d-none');
...
function copyBillingToShipping() {
    ...
    fields.forEach(field => {
        const billingVal = document.querySelector(`input[name="billing_${field}"]`).value;
        const shippingInput = document.querySelector(`input[name="shipping_${field}"]`);
        if (shippingInput) {
...
function resetForm(){
    // Re-enable everything first in case it was disabled by viewPO
    $('#po_form').find('input, select, textarea, button').prop('disabled', false);
    
    document.getElementById('po_form').reset();
    document.getElementById('po_form').action = '<?= $basePath ?>/controller/purchase/add_purchase_order.php';
    
    document.getElementById('items_body').innerHTML = '';
    addItemRow();
    
    document.getElementById('edit_po_id').value = '';
    document.querySelector('.card-title').innerText = 'New Purchase Order';
...
<!-- Add Vendor Modal -->
<div class="modal fade" id="addVendorModal" tabindex="-1" aria-hidden="true" style="z-index: 9999 !important;">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <form id="ajax_vendor_form" action="<?= $basePath ?>/controller/masters/vendors/add_vendor.php" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row  mb-4">
                             <div class="col-md-1 text-center">
                                <div class="position-relative d-inline-block ">
                                    <img id="vendor_avatar_preview" src="<?= $basePath ?>/public/assets/images/users/default_user_image.png" 
                                         class="rounded-circle object-fit-cover shadow-sm border border-2 border-white bg-secondary-subtle" 
                                         style="width: 60px; height: 60px;"
                                         alt="Avatar Preview">
                                    <label for="vendor_avatar_input" class="position-absolute bottom-0 end-0 bg-primary text-white p-1 rounded-circle cursor-pointer shadow-sm" 
                                           style="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; transform: translate(10%, 10%);"
                                           title="Upload Avatar">
                                        <i class="ti ti-camera"></i>
                                    </label>
                                    <input type="file" name="avatar" id="vendor_avatar_input" class="d-none" accept="image/*" onchange="previewVendorAvatar(this)">
                                </div>
                            </div>
