...
require_once __DIR__ . '/user_wise_collection_report.php';

$emp_name = '';
if ($employee_id > 0) {
    $eS = $conn->prepare("SELECT CONCAT(first_name,' ',last_name) as name, employee_code FROM employees WHERE employee_id = ?");
    $eS->bind_param("i", $employee_id);
    $eS->execute();
    $eS->bind_result($_n, $_c);
    if ($eS->fetch()) $emp_name = "$_n ($_c)";
    $eS->close();
}

$spreadsheet = new Spreadsheet();
$sheet = $spreadsheet->getActiveSheet();
$sheet->setTitle('User Collection Report');
$lastCol = 'C';

$sheet->mergeCells("A1:{$lastCol}1");
$sheet->setCellValue('A1', 'USER WISE DAILY COLLECTION REPORT');
...
if (!empty($emp_name))   $activeFilters[] = ['Employee ',      $emp_name];
...
$headers = ['User (Employee)', 'Total Transactions', 'Total Collection (₹)'];
...
foreach ($reportData as $row) {
    ...
    $sheet->setCellValue('B'.$rowIndex, $row['collection_count']);
    $sheet->setCellValue('C'.$rowIndex, number_format($row['total_collection'], 2, '.', ''));
    $grandTotal += $row['total_collection'];
    $rowIndex++;
}
$rowIndex++;
...
header('Content-Disposition: attachment; filename="user_wise_collection_'.date('Y_m_d').'.xlsx"');
