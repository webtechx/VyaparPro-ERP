...
require_once __DIR__ . '/customer_ledger_report.php';
...
$sheet->setCellValue('A1', 'CUSTOMER LEDGER REPORT');
...
$headers = ['Date', 'Ref No', 'Type', 'Particulars', 'Debit (₹)', 'Credit (₹)', 'Balance (₹)'];
...
$running_balance = ($customer_id > 0) ? $opening_balance : 0;

if ($customer_id > 0) {
    $sheet->setCellValue('A'.$rowIndex, 'Opening Balance');
    $sheet->setCellValue('G'.$rowIndex, number_format(abs($running_balance), 2, '.', '') . ' ' . ($running_balance >= 0 ? 'Dr' : 'Cr'));
...
if (!empty($transactions)) {
    foreach ($transactions as $row) {
        ...
        $sheet->setCellValue('B'.$rowIndex, $row['ref_no'] ?? '');
        $sheet->setCellValue('C'.$rowIndex, $row['type_label'] ?? '');
        $sheet->setCellValue('D'.$rowIndex, $row['type_label'] ?? '');
        $sheet->setCellValue('E'.$rowIndex, $debit  > 0 ? number_format($debit,  2, '.', '') : '');
        $sheet->setCellValue('F'.$rowIndex, $credit > 0 ? number_format($credit, 2, '.', '') : '');
        $sheet->setCellValue('G'.$rowIndex, number_format(abs($running_balance), 2, '.', '') . ' ' . ($running_balance >= 0 ? 'Dr' : 'Cr'));
        $rowIndex++;
    }
}
...
$sheet->setCellValue('E'.$rowIndex, number_format($total_debit,  2, '.', ''));
$sheet->setCellValue('F'.$rowIndex, number_format($total_credit, 2, '.', ''));
$sheet->setCellValue('G'.$rowIndex, number_format(abs($running_balance), 2, '.', '') . ' ' . ($running_balance >= 0 ? 'Dr' : 'Cr'));
$sheet->getStyle("A{$rowIndex}:{$lastCol}{$rowIndex}")->getFont()->setBold(true);
...
header('Content-Disposition: attachment; filename="customer_ledger_'.date('Y_m_d').'.xlsx"');
