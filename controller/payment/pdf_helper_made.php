<?php
// controller/payment/pdf_helper_made.php
// Helper to generate Payment Voucher (Payment Made) HTML and PDF
...
function getPaymentMadeHTML($payment_id, $conn, $organization_id) {
    // 1. Fetch Payment Details with Vendor Info
    $sql = "SELECT pm.*, v.display_name, v.company_name, v.gst_no as gst_number, v.mobile,
            va.address_line1, va.address_line2, va.city, va.state, va.pin_code as pincode
            ...
            WHERE pm.payment_id = ? AND pm.organization_id = ?";
    ...
    $logoDataUri = '';
    if(!empty($logo)){
        $orgCode = $orgData['organization_code'] ?? $orgData['organizations_code'] ?? ''; // Handle varying column names
        // Fix: sometimes code is stored in sessions or different tables. 
        // Let's rely on uploads structure: uploads/{CODE}/organization_logo/{LOGO}
        // If code not in orgData, try fetching? Or Session?
        if(!$orgCode && isset($_SESSION['organization_code'])) $orgCode = $_SESSION['organization_code'];
    ...
    // Vendor Details
    $vendName = $pay['company_name'] ?: $pay['display_name'];
    $addrParts = [];
    if(!empty($pay['address_line1'])) $addrParts[] = $pay['address_line1'];
    if(!empty($pay['address_line2'])) $addrParts[] = $pay['address_line2'];
    $street = implode(', ', $addrParts);
    
    $cityState = [];
    if(!empty($pay['city'])) $cityState[] = $pay['city'];
    if(!empty($pay['state'])) $cityState[] = $pay['state'];
    
    $loc = implode(', ', $cityState);
    if(!empty($pay['pincode'])) $loc .= ' - ' . $pay['pincode'];
    
    $vendAddr = $street;
    if($street && $loc) $vendAddr .= '<br>' . $loc;
    else if(!$street) $vendAddr = $loc;
    
    $vendGst = $pay['gst_no'] ?? $pay['gst_number'] ?? '';
    ...
    <body>
            ...
            <tr>
                ...
                <td width='40%' class='text-right'>
                    ...
                    <table style='margin-top: 5px;'>
                        <tr>
                            <td class='text-right small' style='padding-right: 10px;'>Voucher No:</td>
        ...
        <table>
            <tr>
                <td width='48%' style='padding-right: 2%'>
                    <div class='box-title'>Paid To (Vendor)</div>
                    <div class='from-section'>
                        <div class='fw-bold' style='font-size: 11px; margin-bottom: 3px;'>$vendName</div>
        ...
        <div class='footer'>
            <table width='100%'>
                <tr>
                    <td width='50%'>
                         <div style='font-size: 10px; margin-bottom: 30px;'>Receiver Signature</div>
                         <div class='sig-line' style='float:left; text-align:left;'>Singature & Stamp</div>
                    </td>
...
function generatePaymentMadePDF($payment_id, $conn, $organization_id) {
    $html = getPaymentMadeHTML($payment_id, $conn, $organization_id);
    if(!$html) return null;
